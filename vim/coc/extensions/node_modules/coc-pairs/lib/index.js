    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        function insertPair(character, pair) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                if (samePair && rest[0] == character && rest[1] != character) {
                    // move position
                    yield nvim.eval(`feedkeys("\\<C-G>U\\<Right>", 'int')`);
                    return '';
                }
                if (samePair && pre && isWord(pre[pre.length - 1]))
                    return character;
        ...
        function createBufferKeymap(doc) {
            return __awaiter(this, void 0, void 0, function* () {
                if (!doc)
                    return;
                let pairs = doc.getVar('coc_pairs', null);
                if (!pairs)
                    return;
            ...
            if (col > 1 && !/string/i.test(synname)) {
                ...
                if (col - 1 < buf.length) {
                    ...
                    if (pairs.has(pre) && pairs.get(pre) == next) {
                        yield nvim.eval(`feedkeys("\\<C-G>U\\<right>\\<bs>\\<bs>", 'int')`);
                        return;
                    }
                }
            }
        }
        yield nvim.eval(`feedkeys("\\<bs>", 'int')`);
    });
}
